...
const items = ref([
    ...
    },
    {
        ...
        command: () => router.push('/activity-logs')
    },
    {
        label: 'Interventions',
        icon: 'pi pi-cog',
        command: () => router.push('/workload')
    },
    {
        label: 'Performance',
...
const handleLogout = () => {
    // Use authStore.logout() to properly clear Pinia state
    authStore.logout();
    // Navigate to login
    router.push('/login');
};
    ...
    <Menubar :model="items">
        <template #start>
            <div class="flex items-center">
                <span class="nav-brand">SAV System</span>
            </div>
        </template>
