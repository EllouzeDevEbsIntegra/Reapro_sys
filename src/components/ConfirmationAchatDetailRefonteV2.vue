            ...
            <div class="detail-header-left">
                ...
                <div class="detail-title-block">
                    <div class="detail-title-row">
                        ...
                        <span class="refonte-v2-badge">REFONTE V2</span>
                    </div>
                                            ...
                                            <Popover ref="stockFilterPanel">
                                                <div class="filter-popover">
                                                    ...
                                                    <div class="ca-filter-field">
                                                        <label class="ca-filter-label">Condition</label>
                                                        <div class="ca-seg">
                                                            <button v-for="op in operatorButtons" :key="op.value" type="button"
                                                                class="ca-seg-btn" :class="{ active: frsFilters.stock.operator === op.value }"
                                                                @click="frsFilters.stock.operator = op.value" v-tooltip.bottom="op.title">{{ op.symbol }}</button>
                                                        </div>
                                                    </div>
                                                    <div class="ca-filter-field">
                                                        <label class="ca-filter-label">Valeur</label>
                                                    ...
                                                    <div class="ca-filter-actions">
                                                        <Button label="Effacer" severity="secondary" outlined size="small" @click="resetStockFilter" />
                                            ...
                                            <Popover ref="dateFilterPanel">
                                                <div class="filter-popover">
                                                    ...
                                                    <div class="ca-filter-field">
                                                        ...
                                                        <div class="ca-seg">
                                                            <button v-for="op in operatorButtons" :key="op.value" type="button"
                                                                class="ca-seg-btn" :class="{ active: frsFilters.date.operator === op.value }"
                                                                @click="frsFilters.date.operator = op.value" v-tooltip.bottom="op.title">{{ op.symbol }}</button>
                                                        </div>
                                                    </div>
                                                    <div class="ca-filter-field">
                                                        <label class="ca-filter-label">Valeur</label>
                                                        <DatePicker v-model="frsFilters.date.value" class="filter-full"
                                                            dateFormat="dd/mm/yy" showIcon iconDisplay="input" placeholder="Choisir une date" />
                                                    </div>
                                                    <div class="ca-filter-actions">
                                                        <Button label="Effacer" severity="secondary" outlined size="small" @click="resetDateFilter" />
                                            ...
                                            <Popover ref="quantityFilterPanel">
                                                <div class="filter-popover">
                                                    ...
                                                    <div class="ca-filter-field">
                                                        ...
                                                        <div class="ca-seg">
                                                            <button v-for="op in operatorButtons" :key="op.value" type="button"
                                                                class="ca-seg-btn" :class="{ active: frsFilters.quantity.operator === op.value }"
                                                                @click="frsFilters.quantity.operator = op.value" v-tooltip.bottom="op.title">{{ op.symbol }}</button>
                                                        </div>
                                                    </div>
                                                    <div class="ca-filter-field">
                                                        <label class="ca-filter-label">Valeur</label>
                                                    ...
                                                    <div class="ca-filter-actions">
                                                        <Button label="Effacer" severity="secondary" outlined size="small" @click="resetQuantityFilter" />
...
// V2 : boutons segmentés d'opérateur (symbole + libellé en tooltip)
const operatorButtons = [
    { symbol: '>', value: 'gt', title: 'Supérieur à' },
    { symbol: '≥', value: 'ge', title: 'Supérieur ou égal' },
    { symbol: '=', value: 'eq', title: 'Égal à' },
    { symbol: '≤', value: 'le', title: 'Inférieur ou égal' },
    { symbol: '<', value: 'lt', title: 'Inférieur à' }
]
...
<!-- ════════════════════════════════════════════════════════════════════
     REFONTE UI V2 — style ERP moderne (TailAdmin/Admin Dashboard).
     Surcouche purement visuelle, déclarée en dernier (scoped, prime sur l'existant).
     Aucune logique / aucun appel API modifié.
════════════════════════════════════════════════════════════════════ -->
<style scoped>
/* ── Design system local ── */
.line-detail-container {
    ...
    --ca-warning: #ea580c;
    --ca-radius: 16px;
    --ca-shadow: 0 1px 2px rgba(16, 24, 40, 0.05), 0 2px 8px rgba(16, 24, 40, 0.06);
...
/* Sélection FRS très visible mais élégante */
.modern-table tbody tr.bg-blue-100 {
    background-color: var(--ca-primary-soft) !important;
    box-shadow: inset 4px 0 0 var(--ca-primary);
}
...
/* Inputs ligne + raison harmonisés */
.qty-input,
.reason-select {
    border-radius: 8px;
    border-color: var(--ca-border);
...
/* ════ FILTRES V2 (segmented + popover premium) ════ */
.th-filter-btn {
    border-radius: 7px;
    width: 22px;
    height: 22px;
...
/* Petit point indicateur de filtre actif */
