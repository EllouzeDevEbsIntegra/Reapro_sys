<!--
  Composant PARTAGÉ — Dialog « Historique Der P » (LastInvoicedItemCost, vue SQL).
  Ouvert au clic sur le prix de la colonne « Dern. Achat / Der P » (FRS/EQV/KIT).
  Distinct de l'Historique Prix Achat. Même enveloppe que ArticleStockHistoryDialog
  (largeur min(1280px,98vw), hauteur FIXE min(720px,88vh), header Deep Ocean texte
  blanc, table scrollable interne, footer pagination fixe). NE recréez PAS un dialog local.

  Présentation pure : l'appel API (store.fetchLastInvoicedItemCosts) + la pagination
  restent chez le PARENT, qui passe `entries`/`page`/`totalPages` via props et réagit à go-page.
-->
<template>
  <teleport to="body">
    ...
    <div v-if="visible" class="c2dlg-modal" role="dialog" aria-modal="true">
      <div class="c2dlg-head">
        <div class="c2dlg-title"><b>Historique Der P · {{ itemNo }}</b><span>{{ desc || (totalElements + ' ligne(s)') }}</span></div>
      ...
      <div class="c2dlg-body">
        <table class="c2dlg-table">
          ...
          <tbody>
            <tr v-for="(l, i) in entries" :key="l.systemId ?? i" :title="l.systemId || ''">
              <td class="left mono"><b>{{ l.frs || '—' }}</b></td>
              <td class="left mono" :title="l.refFrs">{{ l.refFrs || '—' }}</td>
              <td class="left mono">{{ l.no || '—' }}</td>
              <td class="num mono"><b>{{ formatNumber(l.lastInvoicedDirectCost, 3) }}</b></td>
              <td class="num"><span v-if="l.quantity != null" class="c2dlg-qbadge">{{ Math.round(l.quantity) }}</span><span v-else class="muted">—</span></td>
              <td class="num mono">{{ formatPoDate(l.lastInvoicedCostDate) }}</td>
            </tr>
            <tr v-if="loading && !entries.length"><td colspan="6" class="c2dlg-empty"><i class="pi pi-spin pi-spinner"></i> Chargement…</td></tr>
            <tr v-else-if="!entries.length"><td colspan="6" class="c2dlg-empty"><i class="pi pi-inbox"></i> Aucun historique</td></tr>
          </tbody>
        </table>
      </div>
...
const emit = defineEmits(['update:visible', 'go-page'])
...
.c2dlg-table td.muted, .c2dlg-table .muted { color: #94a3b8; }
...
.c2dlg-qbadge { display: inline-flex; align-items: center; justify-content: center; min-width: 18px; height: 16px; padding: 0 5px; background: #eff6ff; color: #1d4ed8; border: 1px solid #bfdbfe; border-radius: 9999px; font-size: 0.66rem; font-weight: 800; line-height: 1; font-variant-numeric: tabular-nums; }
