<!--
  Composant PARTAGÉ — Dialog « Lignes commande achat » (purchase order lines).
  Ouvert au clic sur le badge "Cmd" (qtyOnPurchOrder > 0) d'une ligne FRS/EQV/KIT.
  Même enveloppe que ArticleStockHistoryDialog (largeur min(1280px,98vw), hauteur
  FIXE min(720px,88vh), header Deep Ocean texte blanc, table scrollable interne,
  footer pagination fixe). NE recréez PAS un dialog local.

  Présentation pure : l'appel API (store.fetchPurchaseLines) + la pagination restent
  ...
  <teleport to="body">
    ...
    <div v-if="visible" class="c2dlg-modal" role="dialog" aria-modal="true">
      <div class="c2dlg-head">
        <div class="c2dlg-title"><b>Lignes commande achat · {{ no }}</b><span>{{ totalElements }} ligne(s)</span></div>
      ...
      <div class="c2dlg-body">
        <table class="c2dlg-table">
          ...
          <tbody>
            ...
            <tr v-if="loading && !entries.length"><td colspan="9" class="c2dlg-empty"><i class="pi pi-spin pi-spinner"></i> Chargement…</td></tr>
            <tr v-else-if="!entries.length"><td colspan="9" class="c2dlg-empty"><i class="pi pi-inbox"></i> Aucune ligne de commande</td></tr>
          </tbody>
        </table>
      </div>
...
const props = defineProps({
  ...
  sortField: { type: String, default: '' },        // colonne triée (clé = propriété entité)
...
// Colonnes (libellé + clé de tri = propriété entité PurchaseLineSqlServer, triable via Spring Pageable).
const COLS = [
  { key: 'documentNo', label: 'N° Commande', align: 'left', width: '13%' },
  { key: 'buyFromVendorNo', label: 'Fournisseur', align: 'left', width: '10%' },
  { key: 'no', label: 'Référence', align: 'left', width: '12%' },
  { key: 'locationCode', label: 'Magasin', align: 'left', width: '9%' },
  { key: 'orderDate', label: 'Date Cmd', align: '', width: '10%' },
  { key: 'description', label: 'Description', align: 'left', width: '15%' },
  { key: 'quantity', label: 'Qté Cmd', align: 'num', width: '9%' },
  { key: 'qtyFirstConfirmation', label: '1er Conf', align: 'num', width: '11%' },
  { key: 'outstandingQuantity', label: 'Qté Restante', align: 'num', width: '11%' }
]
const sortIcon = (key) => {
...
// Lecture tolérante : champ camelCase puis libellé alternatif (comme getPurchaseLineValue source)
